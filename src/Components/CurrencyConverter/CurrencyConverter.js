...
function LineChart({ baseCurrency, selectedCurrency }) {
  ...
  useEffect(() => {
    fetch(`https://api.frankfurter.app/2020-01-01..2022-01-01?from=${baseCurrency}&to=${selectedCurrency}`)
      .then(response => response.json())
      .then(data => {
        const chartLabels = Object.keys(data.rates);
        const chartData = Object.values(data.rates).map(rate => rate[selectedCurrency]);
